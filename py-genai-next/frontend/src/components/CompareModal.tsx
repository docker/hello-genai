        ...
        {showDiff && canDiff && (
          <div className="rounded-xl border bg-card p-4">
            <div className="mb-2 flex gap-4 text-xs font-medium">
              <span className="text-destructive">− {short(results[0].model)}</span>
              <span className="text-success">+ {short(results[1].model)}</span>
            </div>
            ...
            {results.map((r) => (
              <div key={r.model} className="min-w-0 rounded-xl border bg-card p-3">
                <div className="mb-2 flex items-center gap-2">
                  <Badge variant="muted" className="font-mono text-[0.65rem]">{short(r.model)}</Badge>
